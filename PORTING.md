...
The port intentionally omits Cursor plugin metadata, cache markers, vendored dependencies, the optional automation pack, the proprietary plugin loader, cloud-agent syntax, and the `/loop` command. The original Cursor plugin remains unchanged and should be preferred when using pstack natively inside Cursor.
