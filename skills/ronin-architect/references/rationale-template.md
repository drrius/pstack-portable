...
*The recommended architecture. Data structures first; then how data flows through the signatures. Name the load-bearing decisions. State which invariants are encoded in types, where validation lives, and what the system deliberately does not do. Judge interface depth explicitly. State what complexity the public surface hides, what remains exposed to callers, and why the interface is no larger than needed. Cite the principle behind each decision (e.g., `per ronin-principle-boundary-discipline`); don't restate it.*

## Synthesis decision

*Records which structural direction became the base and why, what was adapted from the other directions, and what was rejected and why.*
