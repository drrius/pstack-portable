---
name: figure-it-out
description: "Design an auditable playbook when no narrower one fits: a large migration, an ambitious multi-part change, or work a human reviews after stepping away. Scales rigor to the task, runs a hypothesis loop, and logs decisions via ronin-review. Use for /figure-it-out, 'figure it out', a large migration, or when no narrower playbook applies."
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with isolated `explore` candidates assigned structurally different directions and a read-only `judge`. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
- Decide what fans out. Every delegated leaf lane in the bespoke plan names one primary task profile from the host contract: `explore`, `implement`, `judge`, `explain`, or `verify`. Parallelize only across genuine seams, and give each `implement` worker its own worktree or branch (the **separate-before-serializing-shared-state** principle skill). Operational coordinators that only route or drain work do not need a profile; assign one if they also execute a leaf task. Don't over-fan.
