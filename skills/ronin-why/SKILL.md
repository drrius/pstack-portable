...
Companion to the `ronin-how` skill. `ronin-how` answers what the code does and how it works. `ronin-why` answers what forces led to its shape.
...
After the Sources Consulted block, if the user's `ronin-why` question is a precursor to actually changing this code, convert the lineage findings into a Preserve / Change / Avoid / Risk constraint set suitable for planning the change.
