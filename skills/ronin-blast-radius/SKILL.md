...
Companion to `ronin-how` and `ronin-why`. `ronin-how` tells you what the code does. `ronin-why` tells you why it's shaped that way. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Use `ronin-why` step 2 to pull the PR and commits.
...
4. Be honest about each risk. Give it a real chance of happening and a real cost if it does. Keep the risks you confirmed; list the ones you checked and cleared separately. Same rules as `ronin-why`. Cite a real `file:line`, a search that finds nothing is still an answer, and never make up a caller or an API.
...
6. For a big or wide change, run the distinct risk slices through `ronin-swarm`. Give isolated `explore` workers read-only investigation slices or `judge` workers evaluation-only slices, then aggregate only findings backed by reproducible evidence. The coverage comes from distinct slices and evidence.
...
Write it through `ronin-unslop`, cite real code, and strip anything private before it goes anywhere public.
