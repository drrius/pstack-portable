...
- Transcript evidence that a `SKILL.md` was read from the canonical project-local `.agents/skills/` tree, global `~/.agents/skills/` tree, or a host adapter's manifest-owned alias
- Worker requests that name a skill path
- Terminal, search, or connected-service calls that match a skill's documented operations
