...
6. For a big or wide change, run the distinct risk slices through `swarm`. Give isolated `explore` workers read-only investigation slices or `judge` workers evaluation-only slices, then aggregate only findings backed by reproducible evidence. Optional model diversity may add perspective, but the coverage comes from the slices.
