...
- **Sub-coordinator.** Durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. It owns its track's units and boards, authors its workers' briefs, and spawns its own workers and verifiers only to the nesting depth supported by the host. Pure coordination is a control-plane role outside task profiles; if a sub-coordinator executes a leaf task, that unit gets its own profile. It rolls up aggregates at wave boundaries and never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Every delegated leaf unit names one primary task profile. Use an isolated execution environment when the host provides one; use the local machine when the unit needs local real-surface control, workspace-scoped transcripts, simulators, IDE state, or local-only authentication. Remote workers cannot assume access to local state, so their briefs inline what they need or point at repository paths. Prefer fewer, broader workers; one `implement` writer per worktree or branch (ronin-principle-separate-before-serializing-shared-state). Run a judgment-laden unit verifier as a fresh-context `verify` worker and record its review separation.
...
- `decisions.tsv` is the trail via the ronin-review skill.
...
PROFILE      one of explore, implement, judge, explain, or verify for a leaf unit;
             omit only for a pure coordination or control-plane role
...
7. **Close.** Drain the final inbox, reconcile every spawned agent to a terminal row (done, abandoned, zombie-reconciled), confirm the predicate on the real artifact, confirm every landed PR has a verdict for its current head SHA, audit the trail per ronin-review including its fresh-context review and provenance, encode recurring corrections into `preferences.md` or the brief template. Leave the store intact; it is the postmortem.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts. A dedicated `verify` worker is for units whose verification is expensive, judgment-laden, or high-blast-radius; give it a fresh context and the real acceptance surface. A verifier whose entire product would be rerunning one command is ceremony, not verification.
...
- Never resume a worker merely to check on it when the host treats resume as a new turn. Probe read-only: the ledger, `units.tsv`, the repository forge when configured, pushed branches, and the host's worker-status surface. Transcript mtime is not liveness.
...
- Retry by mode: cap-hit or oom, respawn with smaller scope; network-drop, retry as-is; tool-error, retry once with the same task profile after checking the tool contract; use another model only when evidence identifies model-specific incompatibility; unknown, retry once. Two retries, then abandon the unit and replan around it.
...
- After an agent-host restart, treat worker liveness as unknown until the host's status surface proves otherwise. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach surviving remote work by PR and branch rather than volatile worker id, respawn missing sub-coordinators from their stored briefs plus current state, drain, and resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
