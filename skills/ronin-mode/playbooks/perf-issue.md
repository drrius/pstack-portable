...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a worker using the configured `deep-code` role; review the diff. If delegation is unavailable, implement serially and disclose the substitution. Capture a post-fix trace.
