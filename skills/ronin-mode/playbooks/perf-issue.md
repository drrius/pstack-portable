...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to an `implement` worker; review the diff. Optional routing may request higher reasoning effort when the trace demands it. If delegation is unavailable, implement serially and disclose the substitution. Capture a post-fix trace.
