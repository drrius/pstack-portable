...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to an `implement` worker with a specific scope; review the diff. Optional routing may request higher reasoning effort for a difficult mechanism. If the host cannot delegate, implement serially and disclose that independent authorship was not exercised.
