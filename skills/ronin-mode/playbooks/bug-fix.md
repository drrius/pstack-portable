...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a worker using the configured `deep-code` role with a specific scope; review the diff. If the host cannot delegate, implement serially and disclose that independent authorship was not exercised.
