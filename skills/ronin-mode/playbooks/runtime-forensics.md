...
2. Reduce the artifact to the smoking gun: the function on the hot path, the retainer chain from the leaked object to a GC root, the loop firing without input. Parse large artifacts in a read-only subagent with the `explore` profile (the **ronin-principle-guard-the-context-window** principle skill). Give it the artifact boundary, non-mutating permissions, verifier, stop condition, and reduced evidence to return. Keep the reduced finding in the main thread. If no separate worker context is available, parse it in the coordinator and disclose that fresh context was not exercised; report concurrency separately.
