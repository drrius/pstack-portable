...
1. Locate the prior trail through the host's workspace-scoped transcript adapter, an explicit user-provided transcript path, a worker URL, or a pushed branch. Never search private home-directory transcript state outside the active workspace or task. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a worker with the `explore` profile and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill). Its brief names the transcript boundary, read-only permissions, verifier, stop condition, and returned timeline evidence. If no separate worker context is available, parse in the coordinator and disclose that fresh context was not exercised; concurrency is not applicable to this single lane. If transcripts are unavailable, continue from durable repository and branch evidence and report the capability gap.
