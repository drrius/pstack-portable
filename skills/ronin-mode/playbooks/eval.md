...
4. **Spawn N parallel `implement` candidates** per the **arena** skill's Phase B. Each works in its own sanitized dir from the same organic prompt. Vary only the skill variant under test; optional model routing is held constant unless model routing itself is the experiment.
5. **Spawn one blinded `judge` worker** per the **arena** skill's Phase C. The judge sees outputs by sanitized label and the rubric, never a model name.
...
7. **Read every candidate output yourself** end to end. Compare to the judge's verdict. Disagreement means the evidence or rubric needs inspection, not that one model can be dismissed as biased. Synthesize.
