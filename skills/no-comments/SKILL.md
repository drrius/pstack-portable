...
1. Launch a comment-only Comment Sicko worker through the host's subagent capability. Its request points to `personas/comment-sicko.md`, this skill, and `HOST_CONTRACT.md`; pass the review scope, permissions to edit comments but not application code, verifier, stop condition, and required report without restating the persona. If delegation is unavailable, run the persona as a separate serial review pass and disclose that independent delegation was not exercised.
