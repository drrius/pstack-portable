...
Before using this skill, locate and read `HOST_CONTRACT.md` from the pstack installation root. From this skill's installed directory, the contract is at `../pstack-core/HOST_CONTRACT.md` (the sibling pstack-core skill; resolve this skill's realpath first if the path does not resolve directly). Use its delegation, persona, and safety rules. If it is unavailable, stop and report that the pstack-core skill is not installed alongside this one.
...
1. Launch a comment-only Comment Sicko worker through the host's subagent capability. Its request points to `../pstack-core/personas/comment-sicko.md`, this skill, and `../pstack-core/HOST_CONTRACT.md`; pass the review scope, permissions to edit comments but not application code, verifier, stop condition, and required report without restating the persona. If delegation is unavailable, run the persona as a separate serial review pass and disclose that independent delegation was not exercised.
