...
- The user said "reflect" or "/ronin-reflect".
...
Ask the active host's transcript capability for the current conversation. An explicit user-provided path is also valid when it is constrained to the current workspace or task. Never infer a host's storage layout or search private home-directory state.
...
Pass each template verbatim, substituting the transcript path or digest where marked. If parallel execution is unavailable, run serial fresh-context workers and disclose lost concurrency. If delegation is unavailable, run the three lenses in the coordinator and report `self-review`.
...
Sanity-check the synthesizer's Accepted list. For any item that would be enforced more reliably by a lint rule, script, metadata flag, or runtime check, move it from Accepted to Backlog. The synthesizer already applies this criterion; this is a final pass before edits land. See the **ronin-principle-encode-lessons-in-structure** principle skill.
