...
3. Mine available chat history. Ask the active host adapter for transcripts in the scoped window, or use the explicit in-scope path the user supplied. Order candidates by real modification time rather than identifier, search the topic first, then read only matching conversations and relevant regions; skip the current conversation plus obvious worker, evaluation, and test noise. For a large corpus, define bounded worker requests using the `fast-code` role and fan them out when delegation is authorized. Each request names its corpus slice, read-only boundary, stop condition, and return schema: topic, the user's goal, decisions, open threads, struggles and corrections, artifacts, and a host-stable conversation citation. If delegation is unavailable, process the slices serially and disclose that fallback. If model routing is unavailable, inherit the current model. For one or two conversations, search directly. Keep raw transcripts out of the main context.
