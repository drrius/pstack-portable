...
- The durable-run line asks the host adapter for a goal, monitor, or wait/wake mechanism. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) re-checks the finish condition on events or a heartbeat; without persistence it writes a resume packet and stops honestly.
