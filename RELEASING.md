...
2. **Verify the pinned runtime.** Record `bun --version` and `bun --revision`, run `bun run install:tools`, then require a green `bun run check`. This covers the 58 native tests, strict typecheck, dependency audit, and complete distribution verifier.
