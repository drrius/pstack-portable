...
2. **Verify both runtime paths.** Run `npm test`, install the tooling package with its lockfile, then run its 52 tests and strict typecheck. If Bun support is claimed for the release, repeat the tooling test and typecheck with the recorded Bun version.
